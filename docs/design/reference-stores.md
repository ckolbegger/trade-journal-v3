...
| **`StorageBinding` shape.** ~~Each store assumes put/get/range-query over opaque records (overview rule 5). `listAccounts`/`listValues`/`listProviders` need range queries or in-memory filtering; exact primitive set deferred.~~ **RESOLVED by the StorageBinding drill-down** ([storage-binding.md](storage-binding.md)): full scans via bound-less `range` (these collections are tiny); `listValues(category)` is a primary-key prefix range on `` `${category}|…` ``. No indexes declared. | [storage-binding.md](storage-binding.md) ✓ |
