...
| **PriceProviderStore (new module).** ~~Provider configuration — list/activate/configure providers. A partition addition; joins the step-5 reference-session cluster (AccountStore + TaxonomyStore). Provisional ~3 ops.~~ **RESOLVED by the step-5 reference session** ([reference-stores.md](reference-stores.md)): 4 ops (`addProvider`, `listProviders`, `setActiveProvider`, `deactivateProvider`); credentials excluded by its own charter-drift test; `'trader'` reserved (minted never, a PriceMarkStore sentinel). | [reference-stores.md](reference-stores.md) ✓ |
...
| **OQ 14 — mark correction invalidating a closed-trade snapshot (audit finding B, exported).** ~~A mark correction changes the unrealized-P&L a closed trade's snapshot was computed against (ADR 0007), but PriceMarkStore has no back-reference to consuming trades (marks are shared, ADR 0002) and must not call across stores (rule 1). ADR 0007 currently scopes snapshot invalidation to fill corrections + calc fixes; whether *mark* corrections join is a downstream decision. The detection (which closed trades used the old mark?) and the response (regenerate the snapshot via `record.closedAt` as `asOf`) both belong to the session that owns the regeneration path.~~ **RESOLVED by the FillEntryCoordinator session** ([fill-entry-coordinator.md](fill-entry-coordinator.md) — `regenerateSnapshots(scope?)`): detection = Closed trades where `date(closedAt) === mark.date` ∧ some fill's instrument matches (a coordinator-side filter over `listTrades` — this store stays consumer-agnostic, ADR 0002/rule 1); response = recompute as-of `closedAt` + `replaceSnapshot`; trigger = the UI calls the idempotent sweep after an `upsertMark` that *overwrote* (history non-empty). PerformanceReporting's cold-snapshot compute (its semantic 6) is the read-only twin of the same recipe. | [fill-entry-coordinator.md](fill-entry-coordinator.md) ✓ |
