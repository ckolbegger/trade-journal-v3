...
| — | **StorageBinding** | seam | ~5 | The internal persistence primitive behind all five stores: put/get/delete/range-query over opaque fact records. Two implementations — in-memory (unit tests) and SQLite (prod — WASM/OPFS per ADR 0011; real-SQLite-in-Node serves the integration tier). Zero business rules. |
