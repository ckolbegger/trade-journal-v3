...
| — | **StorageBinding** | seam | 6 | The internal persistence primitive behind all five stores: put/get/delete over opaque JSON records + `range` (primary-key range or declared-index scan) + `ensureIndex` (setup-time, idempotent) + `transaction` (OQ 9's atomic batch — rollback-on-throw, nesting flattens). The keying model: store-composed keys, whole aggregates as single values (a TradeRecord is ONE record — OQ 9's in-store multi-fact cases collapse to single puts), declared secondary indexes honoring ADR 0006's "cheap indexed filter". Two implementations — in-memory (unit tests) and SQLite (prod — WASM/OPFS per ADR 0011; real-SQLite-in-Node serves the integration tier). Zero business rules: values parsed only for declared index keyPaths, never interpreted. Drilled down — see [design doc](storage-binding.md). |
...
7. **The four coordinators** — last, each thin; their dependencies are fully
   ...
   **The drill-down order is complete, and the seam followed** — all twelve
   domain modules (eleven partition rows plus PriceProviderStore) are
   drilled down, the platform/backend landed as ADR 0011, and the
   StorageBinding seam is pinned ([storage-binding.md](storage-binding.md),
   closing OQ 9's primitive shape and narrowing OQ 8). The domain is fully
   designed. Remaining parked OQs: 15 (reporting-UI session), 17, 18, and
   OQ 8's export-format remainder (backup/restore feature design). Next:
   implementation.
...
| 8 | Backup/export/import: storage-seam fan-out or lifecycle coordinator? **(Narrowed — ADR 0011 + the StorageBinding session)** — full backup is the single-file SQLite database itself (copy the file; outside the seam API entirely), which dissolves the fan-out-vs-coordinator question for backup purposes. The API-level restore path stands: `importTrade`, `importMark`, `importEntry` + `importSchema` (verbatim records); the three reference stores reproduce any backup via live ops, *provided* OQ 8 guarantees id-stability across restore. What remains open: the **export format + cross-device transfer UX** — the PWA's per-device storage (ADR 0011) makes phone ↔ laptop transfer a first-release need, served by export/import over the designed ops; the portable format is not yet designed. | backup/restore feature design |
| 9 | ~~Multi-fact write atomicity (commitPlan spans TradingRecord+Reflection): transaction story?~~ **RESOLVED** → one transaction mechanism, used at two levels (decided in the PlanCommitCoordinator session). **Cross-store**: the coordinator wraps its writes in a `StorageBinding.transaction(...)` — `commitPlan` = `tradingRecord.commit` + `reflection.createPlaceholder` in one transaction; only the coordinator can own this (no single store sees both writes). **Single-store multi-fact**: `commit` (Trade+Plan) and `recordFill`'s close branch (fill+status+closedAt+finalFigures) use the same primitive internally. Rejected: compensating-undo/saga (needs a delete op the system deliberately lacks); accept-torn-write (a Planned trade with a missing *required* pre-entry placeholder breaks the bookend invariant — the trader is never prompted). **Primitive shape pinned by the StorageBinding drill-down**: `transaction<T>(fn: () => T): T` — atomic batch, rollback-on-throw, nesting flattens (single-threaded writer). Refinement from the same session: aggregate-per-trade storage (one value per whole TradeRecord — no cross-trade fill query exists) collapses the single-store multi-fact cases to single puts; the primitive's real work is the cross-store coordinator flows. | fully resolved ✓ — mechanism: PlanCommitCoordinator session; shape + keying model: [storage-binding.md](storage-binding.md) |
