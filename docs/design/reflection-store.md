...
| **OQ 9 — multi-fact write atomicity.** ~~`createEntry` for a market entry writes content + links together; `completePlaceholder` transitions state + writes content. These are single-store multi-fact writes that should be atomic, same class as `recordFill`'s close path and PriceMarkStore's `upsertMark` read-modify-write.~~ **RESOLVED by the StorageBinding drill-down** ([storage-binding.md](storage-binding.md) semantic 3): links live on the entry value (the `linkedToTrade` index reads them), so `createEntry` and `completePlaceholder` are single whole-value puts — trivially atomic under the single-threaded writer; `setMarketLinks` is the same read-modify-write shape. No transaction needed in-store. | [storage-binding.md](storage-binding.md) ✓ |
...
| **StorageBinding shape.** ~~This store assumes put/get/range-query over opaque records (overview rule 5). `listEntries` with multiple optional filters needs either compound range queries or in-memory filtering; the exact primitive set is deferred.~~ **RESOLVED by the StorageBinding drill-down** ([storage-binding.md](storage-binding.md)): declared indexes on `state`, `tradeId`, `fillId`, `level`, `type`, `linkedToTrade`, `at` — `listEntries` narrows via one index scan (the owed filter uses `state`; the day stream uses `at`) then filters the narrowed set in memory. | [storage-binding.md](storage-binding.md) ✓ |
