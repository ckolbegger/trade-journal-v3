...
| **StorageBinding shape.** ~~This store assumes `put/get/delete/range-query` over opaque records (overview rule 5), but the StorageBinding interface itself is not yet pinned.~~ **RESOLVED by the StorageBinding drill-down** ([storage-binding.md](storage-binding.md)): one value per whole TradeRecord aggregate (no cross-trade fill query exists — `commit` and `recordFill`'s close branch are single puts); `listTrades` narrows via one declared index (`status`, `strategy`, `accountId`, `underlying`, or `closedAt`) then filters the narrowed set in memory. | [storage-binding.md](storage-binding.md) ✓ |
| **Multi-fact write atomicity (OQ 9) — RESOLVED.** `commit` writes Trade+Plan together; `recordFill` (close branch) writes fill+status+closedAt+finalFigures together. ~~These are single-store multi-fact writes, atomic via the **same `StorageBinding.transaction` primitive** the coordinators use cross-store.~~ **Refined by the StorageBinding drill-down** ([storage-binding.md](storage-binding.md) semantic 3): with aggregate-per-trade storage, both are **single whole-value puts** — trivially atomic under the single-threaded writer; the `transaction(fn)` primitive is reserved for the coordinators' cross-collection flows. | fully resolved ✓ (mechanism: PlanCommitCoordinator session; shape + refinement: [storage-binding.md](storage-binding.md)) |
