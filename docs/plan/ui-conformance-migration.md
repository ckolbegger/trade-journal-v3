...
Then **rewrite [ui-style.md](../design/ui-style.md)** to describe the new system: it currently prescribes the header nav and indigo palette this work replaces, and under constraint 3 a stale design doc outranks the prototype. This also settles UX.5's open vocabulary question on "Fills" as UI copy: the Trade detail card is retitled "Legs & Executions" (CONTEXT.md's Avoid list for Execution names "fill" without carving out UI copy), decision and rationale recorded in `ui-style.md`.

**Considered and deferred, raised by UX.8's review — "Record fill" as a button label.** The same Avoid-list argument that renamed the card heading applies to this button too, but `/record fill/i` is a live selector in at least 5 unit specs (`RecordFillForm.test.tsx` and callers), so renaming it to "Record execution" is real test-update work this restyle story wasn't scoped for. Left as pre-existing copy. Vocabulary debt, not an oversight — a later story that touches `RecordFillForm` should pick it up alongside its own change, rather than as a drive-by rename.

**Noted by UX.8's review, deliberately not built — the replay chart has no colour assertion anywhere in the suite.** That is what let the same Tailwind v3/v4 colour bug ship twice (the blue P&L line and its legend swatch, then the amber risk line and its swatch), and it is what makes the theme-variable tree-shaking hazard recorded in `ui-style.md`'s Charts § unobservable to CI: if a future story removes the last utility-class use of `blue-600`, `amber-500`, `stone-900` or `stone-200`, the corresponding chart element silently loses its paint and every suite stays green. The cheapest honest coverage is a single e2e assertion that the zero line's computed `stroke` is not `none`. That is a new test with a new justification, correctly out of scope for a restyle story — recorded here so a story that touches `ReplayChart` can pick it up.

**Found during UX.8 acceptance, both pre-existing and neither a UX.8 defect:**

1. **The risk-free rate reads back as a floating-point artifact.** Settings → Implied Volatility → enter `3.5`, save, reload: the field displays **`3.5000000000000004`**. That is what the trader sees, not an internal value — the rate is stored as a decimal fraction and multiplied by 100 on read. `SettingsPage`'s UX.8 diff is `className`-only, so this predates the migration. Wants a one-line format fix from whatever story next touches that field.
2. **A bottom-of-page primary button can sit under the fixed tab bar until the page is scrolled.** On a long form scrolled to the top, `Confirm plan` fell inside the bar's band and a click landed on the Journal tab instead. `main` carries `padding-bottom: 96px` against a 57px bar, so the button is always reachable once scrolled and a human scrolls before clicking — ordinary fixed-bar behaviour, not a regression. Noted because it is the one place in the app where the bar overlaps an action.
3. **A Review checkpoint offered two separate places to enter the same Mark.** ~~`WalkCheckpoint`'s own "Marks needed" card (built from `PriceBook.missingMarks` over the review's per-instrument gap range, per [review.md](../design/review.md)'s sequence — "type prices inline... then `detail(tradeId)`") and `TradeDashboard`'s built-in "Valuation → Today's mark" fallback (`marksMissing` off `Valuations.detail()`) both rendered at once inside the same checkpoint — `WalkCheckpoint.tsx:173` mounted `TradeDashboard` unconditionally, in parallel with its own card, instead of after it. Same fact (e.g. today's AAPL price), two inputs. `TradeDashboard`'s fallback is correct in isolation — it's the sole entry point on the standalone Trade Detail page — the bug is only in how `WalkCheckpoint` composed the two.~~ **Fixed** — `WalkCheckpoint` now mounts `TradeDashboard` only once its own "Marks needed" row for *today* (`asOf`) is gone (filled or skipped); older catch-up rows (a missed day) don't gate it, since valuation only needs today's price and `TradeDashboard` knows nothing about historical gap-filling. A skip that leaves today's instrument still unpriced for valuation surfaces once more through `TradeDashboard`'s own card, correctly the only one asking by then.
4. **"Test this source" gives a misleading error right after saving a pricing-source key.** ~~`PriceBook`'s adapters are built exactly once, at startup, from whatever `pricingSources` were persisted at that moment (`main.tsx:27-28`, `bootstrap.ts:38-51` — deliberate, per that file's own comment: "registration happens once, here, at startup... a saved change needs a reload to take effect"). Settings gives no hint of this: saving a key shows only "Saved.", with no cue a reload is required. So the obvious sequence — type key, Save, Test — runs the test against the already-running `PriceBook`, which still has no adapter registered for the test instrument (or the old key). With nothing supporting `MSFT`, `PriceBook.fetch` puts it in `FetchReport.unsupported`, not `errors` (`price-book.ts:90-95`), so `testPricingSource` (`SettingsPage.tsx:94-110`) fell through its `errors`/`stored` checks straight to the generic "No price returned for MSFT in the last 7 days." — indistinguishable from a genuine no-data window (a weekend/holiday), even though the real cause is "the app hasn't picked up your key yet." User-reported 2026-09-03, confirmed by reloading and re-testing successfully.~~ **Fixed** — `testPricingSource` now checks `report.unsupported` before the other branches and shows a distinct, actionable message ("No pricing source is registered. Reload the page and try again.") instead of the generic no-data one. Built test-first: `SettingsPage.test.tsx`'s `'names the reload requirement when no source is registered at all'` reproduces the exact failing sequence (no source registered against a running PriceBook) and pins the new message; the pre-existing `'shows a message... when the source returns no price for the whole trailing window'` test (a real registered source, genuinely empty window) still passes unchanged, proving the two cases stay distinguishable. Deliberately the cheap, honest fix (a clearer error) rather than making `PriceBook`'s source list live-reloadable — that would remove the need to reload at all, but is a real architecture change to the composition root (`main.tsx`), out of scope for a message fix and not requested.

**Not verified at true phone width.** Chrome clamped at `innerWidth: 666` on this machine, so every narrow pass in this migration was a real viewport at 666px rather than 390px. No horizontal scroll and no clipped text at that width on any screen, but a genuine phone viewport needs CDP device emulation and has never been exercised. Worth one pass on a real device before this is considered mobile-verified.
